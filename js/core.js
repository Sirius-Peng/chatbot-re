...
const loadData = async () => {
    try {
        ...
        // 加载 AI 相关数据
        try {
            const savedAiSettings = await localforage.getItem(getStorageKey('aiSettings'));
            if (savedAiSettings) Object.assign(aiSettings, savedAiSettings);
        } catch(e) { console.warn("AI设置加载失败", e); }

        try {
            const savedTodoItems = await localforage.getItem(getStorageKey('todoItems'));
            if (savedTodoItems && Array.isArray(savedTodoItems)) todoItems = savedTodoItems;
        } catch(e) { console.warn("待办数据加载失败", e); }

        try {
            const savedMenstrualData = await localforage.getItem(getStorageKey('menstrualCycleData'));
            if (savedMenstrualData) Object.assign(menstrualCycleData, savedMenstrualData);
        } catch(e) { console.warn("经期数据加载失败", e); }

        try {
            const savedAnniversaries = await localforage.getItem(getStorageKey('anniversaries'));
            if (savedAnniversaries && Array.isArray(savedAnniversaries)) anniversaries = savedAnniversaries;
        } catch(e) { console.warn("纪念日数据加载失败", e); }
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'aiSettings',             val: () => localforage.setItem(getStorageKey('aiSettings'), aiSettings) },
        { key: 'todoItems',              val: () => localforage.setItem(getStorageKey('todoItems'), todoItems) },
        { key: 'menstrualCycleData',     val: () => localforage.setItem(getStorageKey('menstrualCycleData'), menstrualCycleData) },
