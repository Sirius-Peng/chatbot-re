document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        updateLoader('正在初始化 AI 引擎...', '55%');
        try { PromptManager?.init(); } catch(e) { console.warn('PromptManager init failed:', e); }
        ...
        // 初始化音乐播放器
        try { if (typeof initMusicPlayer === 'function') initMusicPlayer(); } catch(e) { console.warn('Music player init failed:', e); }
        ...
        if (myStickerQuickUpload) {
            myStickerQuickUpload.addEventListener('change', async (e) => {
                ...
                for (const file of validFiles) {
                    try {
                        const base64 = await optimizeImage(file, 300, 0.8);
                        myStickerLibrary.unshift(base64);
                        ok++;
                    } catch(err) { fail++; }
                }
                throttledSaveData();
