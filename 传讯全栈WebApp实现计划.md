...
- **源码权威位置待统一**: `chatbot/` 是有提交历史的主项目；根目录工作区包含 2026-06-13 更新的整合代码。后续需要决定是将根目录作为新主线，还是把根目录成果迁回 `chatbot/` 并提交。

### 当前主要风险

- **主线分叉风险**: 根目录和 `chatbot/` 文件差异很大，继续并行会导致测试、构建、部署和提交历史互相脱节。
...
1. **P0: 统一主线与构建入口**
   - 选择根目录或 `chatbot/` 作为唯一权威源码。
   - 同步 `AGENTS.md` / `CLAUDE.md` 中的实际路径说明。
   - 确保 `npm run build`、`npm run test:web`、GitHub Pages workflow 指向同一份源码。
